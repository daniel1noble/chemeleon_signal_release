...
table_style4 <- function(table){
  table <- table %>%
    ...
    flextable::compose(part = "body", j = 3, i = c(1, 10, 19, 28)+1, value = as_paragraph("Snout-vent Length (SVL)")) %>%
...
table_style10 <- function(table){
  table <- table %>%
    ...
    flextable::compose(part = "body", j = 3, i = c(1, 7, 16, 22)+1, value = as_paragraph("Snout-vent length (SVL)")) %>%
...
table_style13 <- function(table){
  table <- table %>%
    ...
    flextable::compose(part = "body", j = 2, i = c(1, 10)+1, value = as_paragraph("Snout-vent Length (SVL)")) %>%
