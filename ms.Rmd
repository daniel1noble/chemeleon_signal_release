...
# Table S5
    tb5 <- data.frame(rbind(robust_Diff_dl_BR,
                      robust_Diff_dS))
    tb5$parameter <- rownames(tb5)
    rownames(tb5) <- NULL
    
  tb5 <- tb5 %>% mutate(VisualRealm = c("Luminance Contrast", rep("", 5), "Chromatic Contrast", 
                                        rep("", 5))) %>%
