...
PredatorAC <- lmer(dL ~ Predator + Population + Predator:Population + (1|ID), data = Predator) 
summary(PredatorAC)
anova_predators_inter <- anova(PredatorAC)

# No support for an interaction, so can drop interaction. Tells us that predation, regardless of whether we are talking snake or bird is just low in Hawaii, otherwise, we would predict that signals would evolve to match the predatory system 
...
hypthesis_test <- glht(PredatorCC_pred_pop, df = 101, # df to get t-distribution, use number of lizards not data points
...
clubSandwich::coef_test(DispBgrdAC, cluster = DisplayBgrd$ID, vcov =  "CR2")
...
# Robust variance esatimation to correct SE's for fixed effects given spectral curves are used multiple times for generating JNDs for each individuals data. 
clubSandwich::coef_test(DispBgrdCC, cluster = DisplayBgrd$ID, vcov =  "CR2")

DisplayBgrd$pop_back <- with(DisplayBgrd, interaction(Population, Background_pop))
DisplayBgrd_pop_backCC <- lmer(dS ~ pop_back + (1|ID), data = DisplayBgrd) 

hypthesis_test_preppop_CC <- glht(DisplayBgrd_pop_backCC, df = 69-1, # df to get t-distribution, use number of lizards not data points
             linfct = mcp(pop_back = c("Hawaii.Kenya  -Hawaii.Hawaii = 0")))
...
# Robust variance esatimation to correct SE's for fixed effects given spectral curves are used multiple times for generating JNDs for each individuals data. 
clubSandwich::coef_test(CourtshipAC, cluster = CourtshipBgrd$ID, vcov =  "CR2")


hypthesis_test_preppopCourtship <- glht(CourtshipBgrd_pop_back, df = 69-1, # df to get t-distribution, use number of lizards not data points
             linfct = mcp(pop_back = c("Hawaii.Kenya  -Hawaii.Hawaii = 0")))
...
# Robust variance esatimation to correct SE's for fixed effects given spectral curves are used multiple times for generating JNDs for each individuals data. 
clubSandwich::coef_test(CourtshipCC, cluster = CourtshipBgrd$ID, vcov =  "CR2")
...
hypthesis_test_preppopCourtshipCC <- glht(CourtshipBgrd_pop_backCC, df = 69-1, # df to get t-distribution, use number of lizards not data points
             linfct = mcp(pop_back = c("Hawaii.Kenya - Hawaii.Hawaii = 0")))
...
```{r Figure2, fig.width = 14, fig.height= 9, fig.cap= ""}
##########
# Data
##########
