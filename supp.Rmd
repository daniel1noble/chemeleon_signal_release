...
##########
    ...
    # Check robustness of results to non-indepenedence using Robust Variance Estimator (RVE)
      robust_DispBgrdCC <- clubSandwich::coef_test(DispBgrdCC_inter, cluster = DisplayBgrd_Hawaii$ID, vcov =  "CR2")
...
##########
  ...
  intearction_AC_Court <- Wald_test(CourtshipAC_inter, constraints = constrain_zero(6:8), 
                                  cluster = CourtshipBgrd_Hawaii$ID, vcov = "CR2")
    

  # Check robustness of results to non-indepenedence using Robust Variance Estimator (RVE)
   robust_CourtshipAC <- clubSandwich::conf_int(CourtshipAC_inter, cluster = CourtshipBgrd_Hawaii$ID, vcov =  "CR2")
  ...
  # Planned contrasts / hypothesis tests. Need to fit model a little differently
    CourtshipBgrd_pop_back_BR <- lmer(dL ~ -1 + pop_back_BR + (1|ID), data = CourtshipBgrd_Hawaii) 
    
   # Fit Robust model
    robust_CourtshipBgrd_pop_back_BR <- clubSandwich::conf_int(CourtshipBgrd_pop_back_BR, 
                                                            cluster = CourtshipBgrd_Hawaii$ID, 
                                                            vcov =  "CR2")
   
    # Contrasts hypothesis tests, but use RVE ********** PROBLEM WITH CONVERGENCE NON-POSITIVE DEF MATRICES
      result_CourtshipBgrd_pop_back_BR <- Wald_test(CourtshipBgrd_pop_back_BR, 
                                                 constraints = constrain_pairwise(c(1:8)), 
                                                 vcov="CR2", tidy = TRUE)
...
##########
  ...
  # Fit main effect model
      CourtshipCC <- lmer(dS ~ Population  + Background_pop + BodyRegion + (1|ID), data=CourtshipBgrd)
      summary(CourtshipCC)
  ...
  robust_CourtshipCC <- clubSandwich::coef_test(CourtshipCC, cluster = CourtshipBgrd$ID, vcov =  "CR2")
   ...
   # Wald tests
           ...
           BodyRegion_CC_Court <- Wald_test(CourtshipCC, constraints = constrain_zero(4:6), 
                                  cluster = CourtshipBgrd$ID, vcov = "CR2")
