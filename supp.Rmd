...
##########
  ...
  DispBgrdAC <- lmer(dL ~ Population  + Background_pop + BodyRegion + (1|ID), data=DisplayBgrd)
  summary(DispBgrdAC)
  ...
  BodyRegion_AC_Disp <- Wald_test(DispBgrdAC, constraints = constrain_zero(4:6), 
                                  cluster = DisplayBgrd$ID, vcov = "CR2")
  
  # Planned contrast. Fit the model in a slightly different way ********** PROBLEMS HERE TO WORK OUT WITH CS
  ...
  DisplayBgrd_pop_back_region <- lmer(dL ~ pop_back_region + (1|ID), data = DisplayBgrd_Hawaii) 
  summary(DisplayBgrd_pop_back_region)
  
  robust_DisplayBgrd_pop_back_region <- clubSandwich::conf_int(DisplayBgrd_pop_back_region, 
                                                               cluster = DisplayBgrd_Hawaii$ID, 
                                                               vcov =  "CR2")
   
  # Contrasts hypothesis tests, but use RVE 
    result_DisplayBgrd_pop_back_region <- Wald_test(DisplayBgrd_pop_back_region, 
                                             constraints = constrain_pairwise(c(1:16)),
                                             vcov="CR2", tidy = TRUE)
    result_DisplayBgrd_pop_back$p_val_bonfer <- sapply(p.adjust(result_DisplayBgrd_pop_back$p_val, "bonferroni"),
...
##########
    ...
    # Fit the main effect model
          DispBgrdCC <- lmer(dS ~ Population  + Background_pop  + BodyRegion + (1|ID), data=DisplayBgrd)
          summary(DispBgrdCC)
    ...
    # Check significance of each factor
        ...
        BodyRegion_CC_Disp <- Wald_test(DispBgrdCC, constraints = constrain_zero(4:6), 
                                  cluster = DisplayBgrd$ID, vcov = "CR2")
...
##########
  ...
  # Fit main effects model
    CourtshipAC <- lmer(dL ~ Population  + Background_pop + BodyRegion + (1|ID), data=CourtshipBgrd)
    summary(CourtshipAC)
  ...
  CourtshipBgrd_pop_back_BR_CC <- lmer(dS ~ -1 + pop_back_BR + (1|ID), data = CourtshipBgrd_Hawaii) 
    ...
    # Contrasts hypothesis tests, but use RVE 
      ...
      string <- c("pop_back_BRHawaii.Kenya.gular - pop_back_BRHawaii.Hawaii.gular|pop_back_BRHawaii.Kenya.midflank - pop_back_BRHawaii.Hawaii.midflank|pop_back_BRHawaii.Kenya.tailbase - pop_back_BRHawaii.Hawaii.tailbase|pop_back_BRHawaii.Kenya.topflank - pop_back_BRHawaii.Hawaii.topflank")
         ...
         result_CourtshipBgrd_pop_backCC <- result_CourtshipBgrd_pop_back_BR_CC[
